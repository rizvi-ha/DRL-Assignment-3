...
EPS_START    = 1.0
EPS_END      = 0.05
EPS_DECAY    = 2_000_000     # linear decay steps
...
def make_env():
    ...
    env = GrayScaleObservation(env, keep_dim=False)  # → (240,256)
    env = ResizeObservation(env, 84)                 # → (84,84)
    env = FrameStack(env, 4)                         # → (4,84,84)
    return env

# ------------------------------------------------------------
# Replay Buffer – pre‑allocated contiguous arrays (no python loops)
# ------------------------------------------------------------
class ReplayBuffer:
    ...
    def sample(self, k):
        ...
        a  = torch.as_tensor(self.action[idx],     device=DEVICE, dtype=torch.int64)
...
# ------------------------------------------------------------
# DQN – same architecture used by original Nature paper
# ------------------------------------------------------------
class DQN(nn.Module):
...
eps   = EPS_START
state = np.array(env.reset(), dtype=np.uint8)  # (4,84,84)
...
for step in progress:
    ...
    if len(replay_buf) > 20_000 and step % 4 == 0:
        ...
        with torch.no_grad():
            next_q     = target_net(n_batch).max(1)[0]
            target_val = r_batch + GAMMA * next_q * (1 - d_batch)
        loss = nn.functional.smooth_l1_loss(q_values, target_val)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    # ---------- end episode ----------
    if done:
        log_f.write(f"Episode {episode}\tReward {cum_reward}\n"); log_f.flush()
        episode += 1
        state = np.array(env.reset(), dtype=np.uint8)
        cum_reward = 0
